...
function syncRepoAndRebuildIndex() {
  ...
  if (changed) {
    ...
    console.log("✅ Repo ist aktuell, vorhandenen Leap-Index verwenden.");
  }

  // Egal was passiert ist: immer Index laden (neu oder alt)
  loadLeapIndex();
}
...
// ===== Start ==========================================================
// 1. Repo syncen + Index ggf. neu bauen + laden
...
app.listen(PORT, () => {
  ...
  console.log(
    `📚 Leap-Index: ${LEAP_INDEX.length} Chunks aus ${LEAP_INDEX_FILE}`
  );
  console.log(`📁 Git-Root: ${REPO_ROOT}`);
  console.log(`📄 Index-Script: ${INDEX_SCRIPT}`);
});